...
import (
	...
	"github.com/abdelrzz9/math_app/backend/internal/module/matrix/usecase"
...
func (h *MatrixHandler) Add(c *gin.Context) {
	...
	result, err := h.uc.Add(req.MatrixA, req.MatrixB)
	if err != nil {
...
func (h *MatrixHandler) Multiply(c *gin.Context) {
	...
	result, err := h.uc.Multiply(req.MatrixA, req.MatrixB)
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, domain.ErrorResponseObj(http.StatusUnprocessableEntity, err.Error()))
		return
	}
...
func (h *MatrixHandler) Determinant(c *gin.Context) {
	...
	result, err := h.uc.Determinant(req.Matrix)
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, domain.ErrorResponseObj(http.StatusUnprocessableEntity, err.Error()))
		return
	}
...
func (h *MatrixHandler) Inverse(c *gin.Context) {
	...
	result, err := h.uc.Inverse(req.Matrix)
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, domain.ErrorResponseObj(http.StatusUnprocessableEntity, err.Error()))
		return
	}
...
func (h *MatrixHandler) Transpose(c *gin.Context) {
	...
	result, err := h.uc.Transpose(req.Matrix)
	if err != nil {
		c.JSON(http.StatusUnprocessableEntity, domain.ErrorResponseObj(http.StatusUnprocessableEntity, err.Error()))
		return
	}
